...
$sql_setup = "CREATE TABLE IF NOT EXISTS password_resets (
...
if (isset($_POST['send_otp'])) {
    ...
    if (!$user_data) {
        ...
        if ($count > 0 && $result['locked_until'] != null && strtotime($result['locked_until']) > time()) {
            ...
            if ($count > 0) {
                // Determine attempts count (each generation increments it)
                $attempts = $result['attempts'];
                if ($attempts >= 3) {
                    $_SESSION['error'] = "Maximum OTP request limit reached. Please try again after 24 hours.";
                    $redirect_url = "login.php";
                } else {
                    $attempts += 1;
                    $update_query = "UPDATE password_resets SET otp = '$new_otp', expires_at = '$expiry_time', attempts = $attempts, locked_until = NULL WHERE email = '$email'";
...
<body>
    ...
    <div class="forgot-shell">
        ...
        <div class="forgot-wrap">
            <div class="forgot-aside">
                <span class="brand-pill"><i class="fas fa-key"></i> Account Recovery</span>
                <h2 class="forgot-title">Reset your password quickly.</h2>
                <p class="mt-3 mb-0" style="color:#aaa;">Enter your registered email and we will send a one-time passcode to continue.</p>
            ...
            <div class="forgot-form-pane">
                <div style="margin-bottom: 30px;">
                    ...
                    <p style="color:#666; font-size:14px;">Enter your registered email to receive an OTP.</p>
                </div>

                <form id="forgotForm" method="post" action="forget-password.php">
                    <div style="margin-bottom: 20px;">
                        <label for="email" class="form-label">Email Address *</label>
                        <input type="email" class="form-control" id="email" name="email" placeholder="you@email.com" required>
                        <span style="color:#E50010; font-size:11px; margin-top:5px; display:none;" id="email_error"></span>
                    </div>
                    ...
                    <button type="submit" class="btn-gradient" id="sendOtpButton">Send OTP</button>

                    <div style="text-align:center; margin-top:20px;">
                        <p style="color:#666; font-size:13px;">Remember your password?
    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (responseTrimmed == 'false') {
                            $('#email_error').text('Email is not registered. Please enter a valid account address so we can locate you.').show();
                            $('#email').addClass('error').css('border-color', '#E50010');
                            $('#sendOtpButton').prop('disabled', true);
                        } else if (responseTrimmed == 'true') {
                            $('#email_error').text('').hide();
                            $('#email').removeClass('error').css('border-color', '#28a745');
                            $('#sendOtpButton').prop('disabled', false);
                        } else {
                            $('#email_error').text('Network timeout verifying email.').show();
                            $('#sendOtpButton').prop('disabled', true);
                        }
                    }
                });
            ...
            // Prevent double-click submittal
            $('#forgotForm').on('submit', function() {
                $('#sendOtpButton').prop('disabled', true).text('PROCESSING...');
            });
