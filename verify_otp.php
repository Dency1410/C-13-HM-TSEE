...
if (!isset($_SESSION['forgot_email'])) {
    $_SESSION['error'] = 'Please request a password reset OTP first.';
...
if (isset($_POST['verify_otp_btn'])) {
    ...
    if (strlen($otp) !== 6) {
        $_SESSION['error'] = 'Please enter the 6-digit OTP sent to your email.';
        echo "<script>window.location.href = 'verify_otp.php';</script>";
        exit();
    }

    $sql = "SELECT otp, expires_at FROM password_resets WHERE email = '$email' ORDER BY id DESC LIMIT 1";
    ...
    if (!$token_data) {
        $_SESSION['error'] = 'No OTP request found. Please request a new OTP.';
        echo "<script>window.location.href = 'forget-password.php';</script>";
        exit();
    }
    ...
    if (strtotime($token_data['expires_at']) < time()) {
        ...
        $_SESSION['error'] = 'This OTP has expired (2 minutes limit). Please request a new one.';
        echo "<script>window.location.href = 'verify_otp.php';</script>";
        exit();
    }
    ...
    if ((string) $token_data['otp'] !== $otp) {
        ...
        // Log the failure to hit the 3-limit tracking
        mysqli_query($conn, "UPDATE password_resets SET attempts = attempts + 1 WHERE email = '$email'");
        
        // Let's check if it just hit the max limit
        $attempt_check = mysqli_fetch_assoc(mysqli_query($conn, "SELECT attempts FROM password_resets WHERE email = '$email' ORDER BY id DESC LIMIT 1"));
        if ($attempt_check['attempts'] >= 3) {
             mysqli_query($conn, "UPDATE password_resets SET locked_until = DATE_ADD(NOW(), INTERVAL 24 HOUR) WHERE email = '$email'");
             $_SESSION['error'] = 'Too many failed attempts. You have been locked out for 24 hours.';
    ...
    $_SESSION['success'] = 'OTP Verified! Please enter your new password.';
...
<head>
    ...
    <title>Forgot Password - H&M</title>
...
<body>
    ...
    <!-- ═══════════════════════════════════
         VERIFY OTP SECTION
    ═══════════════════════════════════ -->
    <div class="forgot-shell" style="max-width: 500px;">

        <?php if (isset($_SESSION['error'])): ?>
            <div class="alert alert-error" style="background:#fff0f0; color:#721c24; border-left:4px solid #E50010; padding:12px; margin-bottom: 20px;">
                <?php 
                    echo $_SESSION['error']; 
        ...
        <?php if (isset($_SESSION['success'])): ?>
            <div class="alert alert-success" style="background:#eaffea; color:#155724; border-left:4px solid #28a745; padding:12px; margin-bottom: 20px;">
                <?php 
        ...
        <div class="forgot-wrap" style="flex-direction: column; min-height: auto;">
            <div class="forgot-form-pane" style="width: 100%; padding: 3rem 2.5rem;">
                <div style="margin-bottom: 20px; text-align: center;">
                    <h3 class="forgot-heading">Verify OTP</h3>
                ...
                <form action="verify_otp.php" method="POST" id="verifyOtpForm">
                    <div style="margin-bottom: 20px;">
                        <label for="otp" class="form-label" style="text-align: left;">OTP Code</label>
                        <div style="display: flex; justify-content: space-between; gap: 8px;">
                            <input type="text" class="form-control otp-digit" style="text-align: center; font-size: 20px; font-weight: bold; padding: 12px 0;" maxlength="1" inputmode="numeric" required>
                            <input type="text" class="form-control otp-digit" style="text-align: center; font-size: 20px; font-weight: bold; padding: 12px 0;" maxlength="1" inputmode="numeric" required>
                            <input type="text" class="form-control otp-digit" style="text-align: center; font-size: 20px; font-weight: bold; padding: 12px 0;" maxlength="1" inputmode="numeric" required>
                            <input type="text" class="form-control otp-digit" style="text-align: center; font-size: 20px; font-weight: bold; padding: 12px 0;" maxlength="1" inputmode="numeric" required>
                            <input type="text" class="form-control otp-digit" style="text-align: center; font-size: 20px; font-weight: bold; padding: 12px 0;" maxlength="1" inputmode="numeric" required>
                    ...
                    <input type="hidden" name="verify_otp_btn" value="1">
                    <button type="submit" class="btn-gradient">Verify</button>

                    <div style="text-align:center; margin-top: 15px; color: #E50010; font-size: 12px; font-weight: 600;" id="timer"></div>

                    <div style="text-align:center; margin-top:20px;">
                        <button type="button" id="resend_otp" style="background: none; border: 1px solid #ccc; padding: 8px 16px; border-radius: 2px; cursor: pointer; transition: all 0.2s;" disabled>Regenerate OTP</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // ═══════════════════════════════════════════════════════════════
            //  VERIFY OTP JS HANDLER (Countdown and 6-Box Logic)
            // ═══════════════════════════════════════════════════════════════
            ...
            const updateTimer = () => {
                ...
                if (secondsLeft > 0) {
                    ...
                    resendButton.style.borderColor = "#ddd";
                    resendButton.style.cursor = "not-allowed";
                    timerDisplay.textContent = `Regenerate OTP in ${secondsLeft} seconds`;
                } else {
                    resendButton.style.color = "#000";
                    resendButton.style.borderColor = "#000";
                    resendButton.style.cursor = "pointer";
                    timerDisplay.textContent = 'You can now regenerate OTP.';
                }
