    ...
    if ($result->num_rows == 1) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            if ($user['is_verified'] == 0) {
                ...
                // Redirect based on role and set separate session data
                if ($user['role'] == 'admin') {
                    $_SESSION['admin_id']     = $user['id'];
                    $_SESSION['admin_name']   = $user['full_name'];
                    $_SESSION['user_role']    = 'admin';
                    $_SESSION['admin_avatar'] = $user['profile_photo'] ?? '';
...
<body>
    ...
    <!-- ═══════════════════════════════════
         H&M HEADER NAVIGATION (from home page)
    ═══════════════════════════════════ -->
    ...
    <div class="login-wrapper">
        <div class="modal-content">
            ...
            <!-- LOGIN SECTION -->
            <div class="panel-section active" id="loginSection">
                <h2 class="section-title">LOG IN</h2>
                <p class="section-subtitle">Welcome back! Log in to access your account.</p>
                ...
                <form id="loginForm" method="POST" action="login.php" novalidate>
                    <div class="form-group">
                        ...
                        <input type="email" class="form-input" id="loginEmail" name="loginEmail" placeholder="you@email.com"
                               data-validation="required,email">
                        <small id="loginEmail_error" class="small text-danger" style="display:none;"></small>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <small id="loginPassword_error" class="small text-danger" style="display:none;"></small>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Login validation is handled by js/form-validation.js
            // We attach a submit handler to trigger validation visually on submit attempt.
            ...
            // ─── URL param & session check ──────────────────────────────────
            const params = new URLSearchParams(window.location.search);
            const view = params.get('view');
            if (view === 'register') window.location.href = 'register.php';
            else if (view === 'forgot') window.location.href = 'forget-password.php';

            // PHP check handles profile redirect if needed
            ...
            // Side Panel Controls (from home page)
