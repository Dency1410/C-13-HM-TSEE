...
if (!isset($_SESSION['forgot_email'])) {
    $_SESSION['error'] = 'Session expired. Please start the reset process again.';
    echo "<script>window.location.href = 'forget-password.php';</script>";
    exit();
}

if (empty($_SESSION['forgot_otp_verified'])) {
    $_SESSION['error'] = 'Please verify your OTP before resetting the password.';
...
if (isset($_POST['reset_pwd_btn'])) {
    ...
    $update_sql = "UPDATE users SET password = '$hashed' WHERE email = '$email'";
...
?>
        <script>
            sessionStorage.removeItem('otpTimerEndAt');
...
<body>
        ...
        <div class="forgot-wrap" style="flex-direction: column; min-height: auto;">
            <div class="forgot-form-pane" style="width: 100%; padding: 3rem 2.5rem;">
                <div style="margin-bottom: 20px; text-align: center;">
                    ...
                    <p style="color:#666; font-size:14px;">Create a new secure password for your account.</p>
                </div>

                <div class="forgot-info" style="border-left-color: #000; background: #fafafa;">
                    <i class="fas fa-shield-alt me-2" style="color: #000;"></i>
                    Password must be at least 6 characters long.
                </div>

                <form id="reset_password_form" method="post" action="reset_password.php">
                    <div style="margin-bottom: 20px;">
                        <label for="new_password" class="form-label" style="text-align:left;">New Password</label>
                        <input type="password" class="form-control" id="new_password" name="new_password" placeholder="Enter new password" required>
                        <span id="new_password_error" style="color:#E50010; font-size:11px; margin-top:5px; display:none;"></span>
                    </div>

                    <div style="margin-bottom: 25px;">
                        <label for="confirm_password" class="form-label" style="text-align:left;">Confirm New Password</label>
                        <input type="password" class="form-control" id="confirm_password" name="confirm_password" placeholder="Confirm new password" required>
                        <span id="confirm_password_error" style="color:#E50010; font-size:11px; margin-top:5px; display:none;"></span>
                    </div>

                    <input type="hidden" name="reset_pwd_btn" value="1">
                    <button type="submit" class="btn-gradient" id="resetPassBtn">Reset Password</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // ═══════════════════════════════════════════════════════════════
            //  RESET PASSWORD JS HANDLER 
            // ═══════════════════════════════════════════════════════════════

            $('#reset_password_form').on('submit', function(e) {
                var newP = $('#new_password').val();
                var confP = $('#confirm_password').val();
                var valid = true;

                if(newP.length < 6) {
                    $('#new_password_error').text('Password must be at least 6 characters.').show();
                    $('#new_password').css('border-color', '#E50010');
                    valid = false;
                } else {
                    $('#new_password_error').hide();
                    $('#new_password').css('border-color', '#28a745');
                }

                if(newP !== confP) {
                    $('#confirm_password_error').text('Passwords do not match.').show();
                    $('#confirm_password').css('border-color', '#E50010');
                    valid = false;
                } else {
                ...
                if(!valid) {
                    ...
                    $('#resetPassBtn').prop('disabled', true).text('PROCESSING...');
                }
            });
