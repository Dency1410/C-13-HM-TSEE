...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'register') {
    ...
    if (mysqli_num_rows($check_result) > 0) {
        echo json_encode(['success' => false, 'message' => 'Email already registered. Please <a href="login.php" style="color:#E50010;">login</a>.']);
        exit;
    }
    ...
    $insert_query = "INSERT INTO users (full_name, email, password, verification_token,last_login) VALUES ('$full_name', '$email', '$hashed_password', '$token' , NOW())";
    
    if (mysqli_query($conn, $insert_query)) {
        $verify_link = "http://localhost/Projects/verify.php?token=$token&email=" . urlencode($email);
        ...
        try {
            ...
            $mail->Port       = 587; // Default MailHog port
            $mail->SMTPAuth   = true;
            $mail->Username   = 'dencysantoki@gmail.com';
            $mail->Password   = 'avzn rrhi uilh ckqs';
            $mail->SMTPSecure = 'tls';
            ...
            // Content
            $mail->isHTML(true);
            $mail->Subject = 'H&M Registration - Verify Your Email';
            $mail->Body    = "Hello $full_name,<br><br>Thank you for registering at H&M. Please click the link below to verify your email address and activate your account:<br><br><a href='$verify_link'>$verify_link</a><br><br>Best regards,<br>H&M Team";
            $mail->AltBody = "Hello $full_name,\n\nThank you for registering at H&M. Please click the link below to verify your email address and activate your account:\n\n$verify_link\n\nBest regards,\nH&M Team";
        ...
        $dev_msg = "Registration successful! ";
        if ($mail_sent) {
            $dev_msg .= "Please check your email to verify your account.";
        } else {
             $dev_msg .= "<br><br><b>Local Testing Link (PHPMailer failed):</b> <a href='$verify_link' style='color:#E50010; font-weight:bold;'>Click here to Verify Email</a>";
        }
        
        echo json_encode(['success' => true, 'message' => $dev_msg]);
    } else {
...
<body>
    ...
    <div class="register-wrapper">
        <div class="modal-content">
            ...
            <p class="section-subtitle">Create your H&M account and enjoy exclusive benefits.</p>
            ...
            <form id="registerForm" novalidate>
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="firstName" name="firstName" placeholder="Full Name"
                           data-validation="required,min,alphabetic" data-min="3">
                    <small id="firstName_error" class="small text-danger" style="display:none;"></small>
                </div>

                <div class="form-group">
                    <label class="form-label" for="email">Email *</label>
                    <input type="email" class="form-input" id="email" name="email" placeholder="you@email.com"
                ...
                <div class="form-group">
                    ...
                    <input type="password" class="form-input" id="password" name="password" placeholder="Min 8 chars, upper, lower, number, symbol"
                           data-validation="required,strongpassword,confirmpassword">
                    <small id="password_error" class="small text-danger" style="display:none;"></small>
                </div>

                <div class="form-group">
                    <label class="form-label" for="password_confirm">Confirm Password *</label>
                    <input type="password" class="form-input" id="password_confirm" name="confirmPassword" placeholder="Re-enter password"
                           data-validation="required">
                    <small id="confirmPassword_error" class="small text-danger" style="display:none;"></small>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Register validation is handled by js/form-validation.js (loaded after this block).
            // We attach to the form element directly so this handler fires BEFORE
            // form-validation.js's document-level submit listener, giving us full control.
            $('#registerForm').on('submit', function(e) {
                e.preventDefault();         // always stop native/doc-level submit

                var $form   = $(this);
                var isValid = true;

                // Trigger 'change' on every field so form-validation.js
                // runs its per-field check and marks is-invalid / is-valid.
                ...
                if (!isValid) return;    // stop – errors already shown
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            showAlert('register-alert', response.message, 'error');
                        }
                        $submitBtn.prop('disabled', false).text('REGISTER');
                    },
                    error: function() {
                        showAlert('register-alert', 'An error occurred. Please try again later.', 'error');
                        $submitBtn.prop('disabled', false).text('REGISTER');
                    }
                });
            });
            ...
            $(document).on('keydown', function(e) {
                if (e.key === 'Escape') window.location.href = 'home.php';
            });
