$(document).ready(function () {
  function getErrorField(field) {
    ...
    if (!errorField.length) {
      ...
      // Escape field name for use in selector if it contains special characters like []
      var safeId = fieldName.replace(/([\[\]])/g, "\\$1");
      errorField = field.closest("form").find("#" + safeId + "_error");
    }

    // If still not found, try without escaping but using attribute selector (more robust)
    if (!errorField.length && fieldName) {
      errorField = field.closest("form").find("[id='" + fieldName + "_error']");
    }
  ...
  function validateInput(input) {
    ...
    // Skip fields that don't need validation to avoid unnecessary processing and potential errors
    var validationType = field.data("validation");
    var isRequired = field.prop("required");
    var hasNativeConstraints = field.attr("minlength") || field.attr("maxlength") || field.attr("min") || field.attr("max") || field.attr("type") === "email" || field.attr("type") === "number";

    if (!validationType && !isRequired && !hasNativeConstraints && field.prop("tagName").toLowerCase() !== "select") {
      return true;
    }

    validationType = (validationType || "").toString().toLowerCase();
  ...
  $(document).on("submit", "form", function (e) {
    ...
    if (!isValid) {
      ...
      if (firstInvalidField) {
        ...
        // For file inputs triggered by a button, focusing the container might be better
        if ($(firstInvalidField).attr("type") === "file") {
          var zone = $(firstInvalidField).closest(".upload-zone");
          if (zone.length) zone[0].scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
      }
