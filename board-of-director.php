...
// Fetch board members
$members = [];
$res = mysqli_query($conn, "SELECT * FROM board_of_director ORDER BY id ASC");
if ($res) {
    while ($row = mysqli_fetch_assoc($res)) {
        $members[] = $row;
    }
}
// Chunk into arrays of 7 to match original row layout
$memberChunks = array_chunk($members, 7);
...
<head>
    ...
    <title>Corporate Governance - H&M Group</title>
    ...
    <!-- No additional jQuery customisation; Bootstrap 5 handles responsiveness via its grid & utilities.
         Only structural CSS (menus, panels, board) is kept; no media queries altered. 
         But we ensure that Bootstrap classes are not overriding custom layout; keep existing styles. -->
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        /* Board of Directors Section - Moved to TOP */
        .board-section {
        ...
        .board-member {
            ...
            height: 0;
            padding-bottom: 120%;   /* 5:6 portrait ratio - shows face + shoulders */
        }
        ...
        .board-member img {
            ...
            display: block;
            object-fit: cover;
            object-position: center top;  /* always shows the face */
            transition: transform 0.4s ease;
        }

        /* All members: show face from top */
        .board-member img {
        ...
        /* Corporate Governance Section - Now comes SECOND */
        ...
        /* Corporate Info Section (unused but keep) */
        .corporate-info {
            background-color: #fff;
            padding: 80px 40px;
        }
        ...
        /* ═══════════════════════════════════
           RESPONSIVE DESIGN (kept original to preserve layout)
           but we also rely on Bootstrap containers & utilities
        ═══════════════════════════════════ */
...
<body>
    ...
    <!-- BOARD OF DIRECTORS SECTION (FIRST) -->
    <section class="board-section">
        <h1 class="page-title">BOARD OF DIRECTORS</h1>

        <?php foreach ($memberChunks as $chunk): ?>
            <!-- Row of Board Images -->
            <div class="board-grid">
                <?php foreach ($chunk as $member): ?>
                    <?php
                    $imgSrc = htmlspecialchars($member['image']);
                    $memberName = htmlspecialchars($member['name']);
                    // Special Class for Tim Gahnström or specific layout can be implemented if needed
                    $extraClass = (stripos($memberName, 'Tim Gahnström') !== false) ? ' tim-image' : '';
                    ?>
                    <div class="board-member<?= $extraClass ?>"><img src="<?= $imgSrc ?>" alt="<?= $memberName ?>"></div>
                <?php endforeach; ?>
            </div>

            <!-- Row of Board Names -->
            <div class="board-names-grid">
                <?php foreach ($chunk as $member): ?>
                    <div class="board-member-name">
                        <?= htmlspecialchars($member['name']) ?><br><small><?= htmlspecialchars($member['title']) ?></small>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php endforeach; ?>
        ...
        <?php if (empty($members)): ?>
            <p style="padding-left: 20px;">No board members found.</p>
        <?php endif; ?>
    </section>
    ...
    <script>
        // Side Panel Control for Ladies Menu (unchanged)
        ...
        document.querySelectorAll('.board-member img').forEach(img => {
            img.onerror = function () {
                this.style.backgroundColor = '#E50010';
                this.style.display = 'flex';
                this.style.alignItems = 'center';
                this.style.justifyContent = 'center';
                this.alt = 'Board Member Photo';
            };
        });
    </script>
