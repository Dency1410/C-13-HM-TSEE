...
$categories_query = "SELECT * FROM categories WHERE gender='Ladies' ORDER BY name ASC";
...
// Build products query for 'Ladies' gender
$query = "SELECT p.*, c.name as category_name 
          FROM products p 
          LEFT JOIN categories c ON p.category_id = c.id 
          WHERE p.gender = 'Ladies'";
...
<body>
        ...
        <nav class="hm-navbar">
            ...
            <ul class="hm-nav-menu">
                <li id="ladiesMenuItem">
                    <a href="product.php?gender=Ladies" class="active">LADIES</a>
                </li>
                <li id="menMenuItem">
    ...
    <!-- ═══════════════════════════════════
         LADIES PAGE MAIN CONTENT
    ═══════════════════════════════════ -->
    <div class="ladies-page-container">
        <!-- Category Sidebar -->
        <aside class="category-sidebar">
            <h2 class="category-title">Categories</h2>
            <ul class="category-list">
                <li><a href="ladies.php" class="<?= empty($category_slug) ? 'active' : '' ?>">All Products</a></li>
                <?php foreach($categories as $cat): ?>
                    <li><a href="ladies.php?category=<?= $cat['id'] ?>" class="<?= $category_slug == $cat['id'] ? 'active' : '' ?>"><?= htmlspecialchars($cat['name']) ?></a></li>
                <?php endforeach; ?>
            </ul>
        </aside>
