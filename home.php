...
$ladies_products = mysqli_query($conn, "SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id=c.id WHERE p.gender='Ladies' AND p.status='In Stock' ORDER BY RAND() LIMIT 10");
$men_products = mysqli_query($conn, "SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id=c.id WHERE p.gender='Men' AND p.status='In Stock' ORDER BY RAND() LIMIT 10");
$kids_products = mysqli_query($conn, "SELECT * FROM (
    (SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id=c.id WHERE p.gender='Kids Girl' AND p.status='In Stock' ORDER BY RAND() LIMIT 5)
    UNION ALL
    (SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id=c.id WHERE p.gender='Kids Boy' AND p.status='In Stock' ORDER BY RAND() LIMIT 5)
) as combined_kids ORDER BY RAND()");

$home_categories = mysqli_query($conn, "SELECT * FROM home_categories WHERE is_active=1 ORDER BY display_order ASC, id ASC");
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <style>
        * {
        ...
        /* Hide scrollbar for Chrome, Safari and Opera */
        .women-products-scroll::-webkit-scrollbar {
            display: none;
        }
...
<body>
        ...
        <header class="hm-header">
            <nav class="hm-navbar">
                ...
                <div class="hm-icons">
                    ...
                    <button class="hm-icon-btn" aria-label="Cart"
                        onclick="window.location.href='<?= isset($_SESSION["user_id"]) ? "cart.php" : "login.php" ?>'">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="kids-columns">
                    ...
                    <div class="kids-column">
                        ...
                        <ul class="kids-column-menu">
                            ...
                            <li><a href="product.php?gender=Kids&category=25">Jumpsuits & playsuits</a>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="kids-column">
                        ...
                        <ul class="kids-column-menu">
                            ...
                            <li><a href="product.php?gender=Kids&category=30">Sweaters & sweatshirts</a>
                            </li>
                        </ul>
                    </div>
                </div>
    ...
    ═══════════════════════════════════ -->
        <section class="categories-section">
            <div class="categories-header">
                ...
                <p>Discover fashion that fits your style</p>
            </div>

            <div class="categories-grid">
                <?php while ($hc = mysqli_fetch_assoc($home_categories)): ?>
                    <div class="category-card" onclick="window.location.href='<?= htmlspecialchars($hc['link_url']) ?>'">
                        <div class="category-image-wrapper">
                            <?php $cat_img = $hc['image_url'] ? $hc['image_url'] : 'https://via.placeholder.com/800x800?text=No+Image'; ?>
                            <img src="<?= htmlspecialchars($cat_img) ?>" alt="<?= htmlspecialchars($hc['title']) ?>"
                                class="category-image">
                            <div class="category-overlay"></div>
                        </div>
                        <div class="category-content">
                            <h3 class="category-title"><?= htmlspecialchars($hc['title']) ?></h3>
                            <p class="category-description"><?= htmlspecialchars($hc['description']) ?></p>
                            <a href="<?= htmlspecialchars($hc['link_url']) ?>" class="category-button">Shop
                                Now</a>
                        </div>
                    </div>
                <?php endwhile; ?>
                <?php if (mysqli_num_rows($home_categories) == 0): ?>
                    <div style="grid-column: 1/-1; text-align: center; padding: 40px; color: #666;">
                        No categories available. Please add some from the admin panel.
                    </div>
                <?php endif; ?>
            </div>
        </section>
        ...
        <!-- ═══════════════════════════════════
         SHOP FOR WOMEN - HORIZONTAL SCROLL
    ═══════════════════════════════════ -->
        <section class="ladies-shop-section">
            <div class="women-shop-header">
                <h2>Shop for Ladies</h2>
                <p>Trending styles curated just for you</p>
            </div>

            <div class="women-scroll-container">
                <!-- Left Scroll Button -->
                <button class="scroll-btn scroll-btn-left" onclick="scrollProducts('left')">
                ...
                <div class="women-products-scroll" id="womenProductsScroll">
                    <?php while ($lp = mysqli_fetch_assoc($ladies_products)): ?>
                        <div class="women-product-card"
                            onclick="window.location.href='product-detail.php?id=<?= $lp['id'] ?>'">
                            <div style="overflow: hidden; border-radius: 8px 8px 0 0;">
                                <?php $img = $lp['image'] ? $lp['image'] : 'https://via.placeholder.com/300x400'; ?>
                                <img src="<?= htmlspecialchars($img) ?>" alt="<?= htmlspecialchars($lp['name']) ?>"
                                    class="women-product-image">
                            </div>
                            <div class="women-product-info">
                                <div class="women-product-category">
                                    <?= htmlspecialchars($lp['category_name']) ?>
                                </div>
                                <h3 class="women-product-title"><?= htmlspecialchars($lp['name']) ?></h3>
                                <div class="women-product-price">
                                    <span class="women-price-current">$<?= number_format($lp['price'], 2) ?></span>
                                </div>
                                <button class="women-product-btn">VIEW DETAIL</button>
                            </div>
                        </div>
                    <?php endwhile; ?>
                    <?php if (mysqli_num_rows($ladies_products) == 0): ?>
                        <p style="padding: 20px;">No products available at the moment.</p>
                    <?php endif; ?>
                </div>
                <!-- Right Scroll Button -->
                <button class="scroll-btn scroll-btn-right" onclick="scrollProducts('right')">
        ...
        <!-- ═══════════════════════════════════
         SHOP FOR MEN - HORIZONTAL SCROLL
    ═══════════════════════════════════ -->
        <section class="men-shop-section">
            <div class="women-shop-header">
                <h2>Shop for Men</h2>
                <p>Trending styles curated just for you</p>
            </div>
            ...
            <div class="women-scroll-container">
                <!-- Left Scroll Button -->
                <button class="scroll-btn scroll-btn-left" onclick="scrollProductsMen('left')">
                    <i class="fas fa-chevron-left"></i>
                </button>
                ...
                <div class="women-products-scroll" id="menProductsScroll">
                    <?php while ($mp = mysqli_fetch_assoc($men_products)): ?>
                        <div class="women-product-card"
                            onclick="window.location.href='product-detail.php?id=<?= $mp['id'] ?>'">
                            <div style="overflow: hidden; border-radius: 8px 8px 0 0;">
                                <?php $img = $mp['image'] ? $mp['image'] : 'https://via.placeholder.com/300x400'; ?>
                                <img src="<?= htmlspecialchars($img) ?>" alt="<?= htmlspecialchars($mp['name']) ?>"
                                    class="women-product-image">
                            </div>
                            <div class="women-product-info">
                                <div class="women-product-category">
                                    <?= htmlspecialchars($mp['category_name']) ?>
                                </div>
                                <h3 class="women-product-title"><?= htmlspecialchars($mp['name']) ?></h3>
                                <div class="women-product-price">
                                    <span class="women-price-current">$<?= number_format($mp['price'], 2) ?></span>
                                </div>
                                <button class="women-product-btn">VIEW DETAIL</button>
                            </div>
                        </div>
                    <?php endwhile; ?>
                    <?php if (mysqli_num_rows($men_products) == 0): ?>
                        <p style="padding: 20px;">No products available at the moment.</p>
                    <?php endif; ?>
                </div>
                <!-- Right Scroll Button -->
                <button class="scroll-btn scroll-btn-right" onclick="scrollProductsMen('right')">
                    <i class="fas fa-chevron-right"></i>
                </button>
            </div>
        </section>
        ...
        <!-- ═══════════════════════════════════
         SHOP FOR KIDS - HORIZONTAL SCROLL
    ═══════════════════════════════════ -->
        <section class="kids-shop-section">
            <div class="women-shop-header">
                <h2>Shop for Kids</h2>
                <p>Trending styles curated just for you</p>
            </div>
            ...
            <div class="women-scroll-container">
                <!-- Left Scroll Button -->
                <button class="scroll-btn scroll-btn-left" onclick="scrollProductsKids('left')">
                    <i class="fas fa-chevron-left"></i>
                </button>
                ...
                <div class="women-products-scroll" id="kidsProductsScroll">
                    <?php while ($kp = mysqli_fetch_assoc($kids_products)): ?>
                        <div class="women-product-card"
                            onclick="window.location.href='product-detail.php?id=<?= $kp['id'] ?>'">
                            <div style="overflow: hidden; border-radius: 8px 8px 0 0;">
                                <?php $img = $kp['image'] ? $kp['image'] : 'https://via.placeholder.com/300x400'; ?>
                                <img src="<?= htmlspecialchars($img) ?>" alt="<?= htmlspecialchars($kp['name']) ?>"
                                    class="women-product-image">
                            </div>
                            <div class="women-product-info">
                                <div class="women-product-category">
                                    <?= htmlspecialchars($kp['category_name']) ?>
                                </div>
                                <h3 class="women-product-title"><?= htmlspecialchars($kp['name']) ?></h3>
                                <div class="women-product-price">
                                    <span class="women-price-current">$<?= number_format($kp['price'], 2) ?></span>
                                </div>
                                <button class="women-product-btn">VIEW DETAIL</button>
                            </div>
                        </div>
                    <?php endwhile; ?>
                    <?php if (mysqli_num_rows($kids_products) == 0): ?>
                        <p style="padding: 20px;">No products available at the moment.</p>
                    <?php endif; ?>
                </div>
                <!-- Right Scroll Button -->
                <button class="scroll-btn scroll-btn-right" onclick="scrollProductsKids('right')">
                    <i class="fas fa-chevron-right"></i>
                </button>
            </div>
        </section>
    ...
    ═══════════════════════════════════ -->
        <footer class="hm-footer">
            <div class="footer-container">
                ...
                <div class="footer-copyright">
                    <p>The content of this site is copyright-protected and is the property of H & M
                        Hennes & Mauritz AB.</p>
                </div>
            </div>
        </footer>
        ...
        <script>
            ...
            // Keep ladies panel open when hovering over it
            ladiesSidePanel.addEventListener('mouseenter', function () {
                ladiesSidePanel.classList.add('active');
                panelOverlay.classList.add('active');
            });

            // Close ladies panel when mouse leaves
            ...
            // Close ladies panel when clicking back arrow
            closeLadiesPanelBtn.addEventListener('click', function () {
                closeAllPanels();
            });

            // MEN MENU CONTROLS
            // Open men panel on hover
            ...
            // Keep men panel open when hovering over it
            menSidePanel.addEventListener('mouseenter', function () {
                menSidePanel.classList.add('active');
                panelOverlay.classList.add('active');
            });

            // Close men panel when mouse leaves
            ...
            // Close men panel when clicking back arrow
            ...
            // Keep kids panel open when hovering over it
            kidsSidePanel.addEventListener('mouseenter', function () {
                kidsSidePanel.classList.add('active');
                panelOverlay.classList.add('active');
            });

            // Close kids panel when mouse leaves
            ...
            // Close kids panel when clicking back arrow
            closeKidsPanelBtn.addEventListener('click', function () {
                closeAllPanels();
            });

            // Horizontal Scroll Function for Women's Products
            function scrollProducts(direction) {
                const container = document.getElementById('womenProductsScroll');
            ...
            // Horizontal Scroll Function for Men's Products
            function scrollProductsMen(direction) {
                const container = document.getElementById('menProductsScroll');
                const scrollAmount = 320;
            ...
            // Horizontal Scroll Function for Kids' Products
            function scrollProductsKids(direction) {
                const container = document.getElementById('kidsProductsScroll');
                const scrollAmount = 320;
